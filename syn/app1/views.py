...
from django.http import HttpResponseBadRequest, FileResponse, Http404, JsonResponse, request, HttpResponse, StreamingHttpResponse
...
def index(request):
    ...
    return render(request, 'index.html', {
        ...
        'login': 'login' in request.session or request.user.is_authenticated,
    })
...
def get_certificate_status(user, course, quiz=None, all_lessons_completed=None):
    ...
    # Check for a passing result specifically on Level 3
    best_level_3_result = Result.objects.filter(
        user_email=user.email,
        quiz__course=course,
        level=3
    ).order_by("-score", "-submitted_at").first()

    if not best_level_3_result or not best_level_3_result.total:
        return quiz, None, None, False

    certificate_percentage = round((best_level_3_result.score / best_level_3_result.total) * 100, 1)
    certificate_available = all_lessons_completed and certificate_percentage >= 70

    return quiz, best_level_3_result, certificate_percentage, certificate_available
...
def courses(request, id):
    ...
    if is_enrolled:
        passed_levels = list(Result.objects.filter(user_email=user.email, quiz__course=cor, score__gte=7).values_list('level', flat=True))
        max_passed = max(passed_levels) if passed_levels else 0
        current_level = min(max_passed + 1, 3)
        
        for lvl in range(1, 4):
            if lvl <= max_passed:
                status = "Completed"
                locked = False
            elif lvl == current_level and all_lessons_completed:
                status = "Pending"
                locked = False
            else:
                status = "Locked"
...
def enroll_course(request, id):
    ...
    try:
        ...
        # If already enrolled → status
        if enrollment.exists():
            return JsonResponse({'status': 'already_enrolled'})

        # Create Cashfree order
        amount = float(course.price)
        cf_order_id = f"course{course.id}_user{user.id}_{timezone.now():%H%M%S}"
        ...
        return JsonResponse({
            ...
            "cf_order_id": order_data.order_id,
            "amount": amount,
        ...
        return JsonResponse({
            'status': 'error',
            'message': f'Unable to create payment order: {error_message}'
        }, status=502)
...
def send_html_email_with_logo(subject, template_name, context, to_email, pdf_attachment=None, pdf_name=None):
    ...
    logo_path = os.path.join(settings.BASE_DIR, 'app1', 'static', 'img', 'logo-main.png')
    if os.path.exists(logo_path):
        with open(logo_path, 'rb') as img:
            logo = MIMEImage(img.read())
            logo.add_header('Content-ID', '<logo_main>')
            logo.add_header('Content-Disposition', 'inline', filename='logo-main.png')
            email.attach(logo)
            
    if pdf_attachment and pdf_name:
        email.attach(pdf_name, pdf_attachment, "application/pdf")
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        send_html_email_with_logo(
            subject="Welcome to Syntax Academy 🎉",
            template_name="emails/email_register.html",
            context={
                'user_name': name,
                'login_url': login_url
            },
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        # 1. TRY ADMIN LOGIN FIRST (Django Staff)
        from django.contrib.auth.models import User
        django_user = User.objects.filter(username=email).first() or User.objects.filter(email=email).first()
        if django_user:
            admin_user = authenticate(request, username=django_user.username, password=password)
            if admin_user and admin_user.is_staff:
                auth_login(request, admin_user)
                # Redirect to next url or fallback to admin dashboard
                if next_url:
                    return redirect(next_url)
                return redirect(reverse('dashboard_home'))
            else:
                # If it matches an admin user but authentication failed, show incorrect password/email alert
                return render(request, 'login.html', {
                    'wrong': 'Incorrect Password or Email!!!',
                    'next': next_url,
                })

        # 2. TRY STUDENT LOGIN (Registration Model)
        try:
...
def logout(request):
    auth_logout(request) # Logs out Django user
    request.session.flush() # Clears custom session variables
...
def quiz_page(request, course_id, level):
    ...
    # Verify level access
    passed_levels = Result.objects.filter(user_email=user.email, quiz__course=course, score__gte=7).values_list('level', flat=True)
    max_passed = max(passed_levels) if passed_levels else 0
    max_allowed_level = min(max_passed + 1, 3)

    if level > max_allowed_level:
        messages.warning(request, f"You must pass Level {max_allowed_level} first!")
        return redirect('courses', id=course.id)

    all_questions = list(Question.objects.filter(quiz__course=course).order_by('id'))
    
    if level == 1:
        questions = all_questions[0:10]
    elif level == 2:
    ...
    return render(request, "quiz.html", {
        "quiz": quiz,
        "course": course,
        "quiz_title": f"{course.name} - Level {level}",
        "questions": questions,
        "level": level
    })
    
    
def submit_quiz(request, course_id, level):
    ...
    if request.method == "POST":
        ...
        # Send certificate if passed level 3
        if level == 3 and percentage >= 70 and all_lessons_completed:
            try:
                pdf_bytes = build_certificate_pdf(user, course, result, percentage)
                
                send_html_email_with_logo(
                    subject=f"Your Certificate for {course.name} is here! 🎓",
                    template_name='emails/email_certificate.html',
                    context={
                        'user_name': user.name,
                        'course_name': course.name,
                        'score': percentage,
                    ...
                    pdf_name=f"certificate_{course.id}.pdf"
                )
            except Exception as e:
                print("Certificate Email Error:", str(e))
...
def about(request):
    if 'login' in request.session or request.user.is_authenticated:
...
def forgot_password(request):
    if request.method == 'POST':
        ...
        try:
            ...
            send_html_email_with_logo(
                subject='Syntax Academy Password Reset OTP',
                template_name='emails/email_forgot_password.html',
                context={
                    'user_name': user.name,
                    'otp': otp
                },
...
def reset_password(request):
    if request.method == 'POST':
        ...
        send_html_email_with_logo(
            ...
            template_name='emails/email_reset_success.html',
            context={
                'user_name': user.name,
                'login_url': login_url
            },
...
# Cashfree Client Initialization
def get_cashfree_client():
    env = Cashfree.SANDBOX if settings.CASHFREE_ENVIRONMENT == 'SANDBOX' else Cashfree.PRODUCTION
    return Cashfree(
        XEnvironment=env,
...
def create_cashfree_order(amount, customer_id, customer_phone, customer_email, order_id, course_id=None, request=None):
    cashfree = get_cashfree_client()
    
    customer_details = CustomerDetails(
        customer_id=f'USER_{customer_id}',
        customer_phone=str(customer_phone),
        customer_email=customer_email
    )
    
    order_meta = None
    if request and course_id:
        return_url = request.build_absolute_uri(reverse('courses', args=[course_id])) + f"?order_id={order_id}"
...
def get_cashfree_order_status(cf_order_id):
    cashfree = get_cashfree_client()
    try:
        api_response = cashfree.PGFetchOrder("2023-08-01", cf_order_id)
        return api_response.data
    except Exception as e:
        raise Exception(f"Cashfree Fetch Order Failed: {str(e)}")
...
def payment_success(request):
    ...
    try:
        user = Registration.objects.get(email=request.session['login'])
        course_id = request.POST.get("course_id")
        cf_order_id = request.POST.get("order_id")

        if not course_id or not cf_order_id:
            return JsonResponse({
                'status': 'error',
        ...
        order_details = get_cashfree_order_status(cf_order_id)
        ...
        if order_details.order_status == "PAID":
            ...
            payment = Payment.objects.create(
                user=user,
                course=course,
                amount=int(order_details.order_amount),
                cf_order_id=cf_order_id
            )

            # Send email
            try:
                send_payment_invoice_email(payment, request)
            except Exception as e:
            ...
            return JsonResponse({
                'status': 'failed',
                'message': f'Payment not successful. Status: {order_details.order_status}'
            })
...
def send_payment_invoice_email(payment, request):
    pdf_bytes = build_receipt_pdf(payment)
    subject = f"Your Syntax Academy invoice for {payment.course.name}"
    
    course_url = request.build_absolute_uri(reverse('courses', args=[payment.course.id]))
    order_id = getattr(payment, 'cf_order_id', payment.id)
    
    send_html_email_with_logo(
        subject=subject,
        template_name='emails/email_payment_success.html',
        context={
        ...
        pdf_attachment=pdf_bytes,
        pdf_name=f"invoice_{payment.id}.pdf"
    )
...
def build_certificate_pdf(user, course, best_result, certificate_percentage):
    from io import BytesIO
    buffer = BytesIO()
...
def download_certificate(request, course_id):
    ...
    pdf_bytes = build_certificate_pdf(user, course, best_result, certificate_percentage)
...
def google_auth(request):
    ...
    # If sent via JSON body (fallback), parse it
    ...
    if not credential:
        messages.error(request, 'Google authentication failed: No credential received.')
        return redirect('login')
    ...
    try:
        ...
        # Force urllib3 to use IPv4 DNS resolution (bypasses slow local IPv6 timeouts)
        try:
            import requests.packages.urllib3.util.connection as urllib3_connection # type: ignore
        except ImportError:
        ...
        # Custom session to bypass local SSL issues
        ...
        # Check if the Google email belongs to a Django staff/admin user
        from django.contrib.auth.models import User
        django_user = User.objects.filter(email=email, is_staff=True).first() or User.objects.filter(username=email, is_staff=True).first()
        if django_user:
        ...
        try:
            ...
            # Send welcome email for new Google signup (asynchronously in a background thread to avoid blocking redirection)
            try:
                import threading
                from django.urls import reverse
                login_url = request.build_absolute_uri(reverse('login'))
                email_thread = threading.Thread(
                    target=send_html_email_with_logo,
                    kwargs={
                        'subject': "Welcome to Syntax Academy 🎉",
                        'template_name': "emails/email_register.html",
                        'context': {
                ...
                print(f"Failed to start welcome email thread to new Google user {email}: {email_err}")

        # Set session — this is the key login mechanism used across the app
        ...
        messages.error(request, 'Google authentication failed: Invalid token.')
        return redirect('login')
    except Exception as e:
