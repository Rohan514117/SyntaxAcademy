    ...
    .then(data => {
        ...
        if(data.status === "payment_required"){
            ...
            const cashfree = Cashfree({
                mode: data.environment.toLowerCase() // "sandbox" or "production"
            });
            ...
            cashfree.checkout(checkoutOptions).then(function(result){
                ...
                // When using _self, the browser will redirect automatically.
                // We don't call verifyPayment here to avoid premature failure popups.
            });
...
function verifyPayment(orderId, courseId) {
    showPaymentStatus("processing", "Checking payment status with Cashfree...");
    ...
    .then(resData => {
        if (resData.status === "success") {
            showPaymentStatus("success", "Welcome to the course! Your access is now active.");
        } else {
            showPaymentStatus("error", resData.message || "Payment verification failed.");
        }
    })
    .catch(err => {
        console.error("Verification Error:", err);
        showPaymentStatus("error", "Network error during verification. Please contact support.");
    });
}
...
function showPaymentStatus(status, message) {
    ...
    const btn = document.getElementById("closePaymentModal");
    ...
    if (status === "processing") {
        ...
        title.innerText = "Verifying Payment";
        msg.innerText = message || "Please wait...";
        btn.style.display = "none";
    } else if (status === "success") {
        icon.className = "payment-icon success";
        title.innerText = "Payment Successful!";
        msg.innerText = message || "You have been successfully enrolled in the course.";
        btn.style.display = "inline-block";
        btn.onclick = () => window.location.href = MY_ENROLLS_URL;
    } else {
        icon.className = "payment-icon error";
        title.innerText = "Payment Failed";
        msg.innerText = message || "Something went wrong. Please try again.";
        btn.style.display = "inline-block";
